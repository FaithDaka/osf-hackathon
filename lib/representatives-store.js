...
// Local-council reps for an area: district-wide office holders
// (subcounty null) plus those posted to the chosen sub-county.
export function getLocalReps(repsData, district, subcounty) {
  return itemsOf(repsData)
    .filter((r) => r.group === 'local_council')
    .filter((r) => (!district || r.district === district))
    .filter((r) => (!r.subcounty || !subcounty || r.subcounty === subcounty))
    .sort((a, b) => String(a.name).localeCompare(String(b.name)));
}
